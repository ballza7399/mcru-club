...
class Club extends Model
{
    ...
    /** รายการชมรมสำหรับหน้าจัดการ (admin และ staff เห็นทั้งหมด, president เห็นเฉพาะของตน) แบบแบ่งหน้า */
    public function listForManage(string $role, int $userId, int $limit = 10, int $offset = 0): array
    {
        $sql = 'SELECT c.*, u.student_id AS pres_id, u.name AS pres_name,
                       ...
                       (SELECT COUNT(DISTINCT u2.faculty) FROM club_members cm2 JOIN users u2 ON cm2.user_id = u2.id WHERE cm2.club_id = c.id AND u2.faculty IS NOT NULL AND u2.faculty != "") AS unique_faculties
                FROM clubs c
                LEFT JOIN users u ON c.president_id = u.id';
